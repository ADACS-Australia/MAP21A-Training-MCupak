...
`check_state.yml` - checks OS and whether specified services are available. 
Note: ansible has a `systemd` module which can be used to ensure services are running.
...
**Note:** The default module for the ansible command-line utility is the `ansible.builtin.command` module. This will run the command and specified arguments on each node. However, commands are not processed via the shell, so variables like **`$HOSTNAME`** and operations like **`"*"`, `"|"`**, etc will not work.

---

### OS package updates / User set up / Restart service

`group.yml` - example of adding new groups, users and sudo rights

`package.yml` - example of installing packages
...
## Example 3 - Automated Software Update - cronjobs
